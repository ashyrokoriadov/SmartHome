// Victronparser.ino - Arduino-compatible port of VictronGate parser
// Designed to run on Arduino Uno R4 WiFi (adjust Serial baud if needed)
...
// Simple fixed-size byte buffer used inside parser
struct ByteBuffer {
  uint8_t data[512];
  int len;
};
...
public:
  // inputTokens: array of String tokens, tokenCount - number of tokens
...
private:
  ByteBuffer ParseToBytes(String tokens[], int tokenCount) {
	ByteBuffer buf; buf.len = 0;
	for (int i = 0; i < tokenCount; ++i) {
	  String t = tokens[i];
	  if (t.length() == 0) continue;
	  // try parse as hex (C# code tries hex first)
	  bool isHex = true;
  ...
  // patterns (as in original C#)
	static const uint8_t VoltagePattern[2]; // { V, TAB }
  static const uint8_t CurrentPattern[2]; // { I, TAB }
...
const uint8_t VictronDataParser::VoltagePattern[2] = { BytesMappings::byteV, BytesMappings::byteTab };
const uint8_t VictronDataParser::CurrentPattern[2] = { BytesMappings::byteI, BytesMappings::byteTab };
const uint8_t VictronDataParser::PanelVoltagePattern[4] = { BytesMappings::byteV, BytesMappings::byteP, BytesMappings::byteV, BytesMappings::byteTab };
const uint8_t VictronDataParser::PanelPowerPattern[4] = { BytesMappings::byteP, BytesMappings::byteP, BytesMappings::byteV, BytesMappings::byteTab };
const uint8_t VictronDataParser::ChargerStatePattern[3] = { BytesMappings::byteC, BytesMappings::byteS, BytesMappings::byteTab };
const uint8_t VictronDataParser::ErrorPattern[4] = { BytesMappings::byteE, BytesMappings::byteR, BytesMappings::byteR, BytesMappings::byteTab };
const uint8_t VictronDataParser::H19Pattern[4] = { BytesMappings::byteH, BytesMappings::hexByte1, BytesMappings::hexByte9, BytesMappings::byteTab };
const uint8_t VictronDataParser::ProductIdPattern[4] = { BytesMappings::byteP, BytesMappings::byteI, BytesMappings::byteD, BytesMappings::byteTab };
const uint8_t VictronDataParser::SerialNumberPattern[5] = { BytesMappings::byteS, BytesMappings::byteE, BytesMappings::byteR, BytesMappings::byteHash, BytesMappings::byteTab };
...
private:
  ...
  ParsedValue AdjustValue(const String &key, const String &val) {
	...
	if (key == "PanelVoltage") {
	  double pv = val.toDouble(); char tmp[32]; dtostrf(pv / 1000.0, 0, 3, tmp); return ParsedValue(key, String(tmp));
	}
	if (key == "PanelPower") {
	  return ParsedValue(key, val);
	}
	if (key == "ChargerState") {
	  int s = val.toInt(); return ParsedValue(key, ChargerStateName(s));
	}
	if (key == "Error") {
	  return ParsedValue(key, val);
	}
	if (key == "H19") {
	  double h = val.toDouble(); char tmp[32]; dtostrf(h / 100.0, 0, 3, tmp); return ParsedValue(key, String(tmp));
	}
	if (key == "ProductId" || key == "SerialNumber" || key == "Unknown") {
	  return ParsedValue(key, val);
	}
	if (key == "GainedEnergy") {
	  double g = val.toDouble(); char tmp[32]; dtostrf(g / 100.0, 0, 3, tmp); return ParsedValue(key, String(tmp));
	}
	return ParsedValue::Empty();
  }
...
void setup() {
  ...
  // Copy PROGMEM string to RAM for tokenization
  char buf[4096];
  strncpy(buf, wholeInput, sizeof(buf));
  buf[sizeof(buf)-1] = '\0';
  ...
  char *p = strtok(buf, " ");
  while (p != NULL && tokenCount < MAX_TOKENS) {
	tokens[tokenCount++] = String(p);
	p = strtok(NULL, " ");
  }
  ...
  for (int i = 0; i < tokenCount; ++i) {
	ParsedValue pv = reader.ReadByte(tokens[i]);
	if (!pv.IsEmpty() && pv.Name != "Unknown") {
	  Serial.println(pv.ToString());
	}
	int d = random(0, 3); // 0..2 ms
	delay(d);
  }
